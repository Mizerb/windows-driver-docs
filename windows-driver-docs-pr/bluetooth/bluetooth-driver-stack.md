...
- **Bluetooth driver stack components**
  ...
  - **BthServ**- A service that is responsible for caching and forwarding inquiry data to Bthport. It also implements the ATT protocol and GATT profile, along with CSIP profile, DIS, and BAS services.  
  ...
  - **BthA2dp** The component that implments the driver side of the Bluetooth Advanced Audio Distrbution Profile in Windows. 
  ...
  - **Microsoft.Bluetooth.Legacy.LEEnumerator**- The Bluetooth Low Energy (LE) bus driver. It is also responsible for creating PDOs to represent the remote devices and their primary services.
 
  - **bthHFEnum**- The enumerator driver for the Hands Free Profile (HFP).
  ...
  - **BthPort**- A minidriver loaded by the BthUsb miniport. BthPort provides four components:
    ...
    2. The SCO component implements the Synchronous Connection-Oriented (SCO) protocol. This protocol supports creating point-to-point connections to a remote device. SCO clients communicate with the SCO interface by [building and sending](building-and-sending-a-brb.md) Bluetooth request blocks (BRBs).
    3. L2CAP implements the Bluetooth logical link control and adaptation protocol. This protocol supports creating a lossless channel to a remote device. L2CAP clients communicate with the L2CAP interface by building and sending Bluetooth request blocks (BRBs).
    4. SDP implements the Bluetooth Service Discovery Protocol.
